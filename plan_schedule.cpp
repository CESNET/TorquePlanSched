...
int check_cluster_suitable(server_info* p_info, plan_cluster* cl, plan_job* job)
  ...
  while (iter != NULL)
	...
	if (iter -> name[0] == '^')
	{
	  ...
	  //if (!is_prefix(string(iter -> name),"q_"))
...
int plan_set_cluster_server(sched* schedule, int clus_number, server_info* sinfo)
  ...
  if (schedule -> clusters[clus_number]->sinfo == NULL)
    {
	sched_log(PBSEVENT_DEBUG2, PBS_EVENTCLASS_REQUEST, schedule -> clusters[clus_number] -> cluster_name,"WARNING! Server info deleted");
    } else
    {
   // sched_log(PBSEVENT_DEBUG2, PBS_EVENTCLASS_REQUEST, "Server info in cluster updated", "cluster %d has server %s",clus_number,schedule->clusters[clus_number]->sinfo->name);
    }
...
sch_resource_t job_get_scratch_local(JobInfo *jinfo)
  ...
  if (spec->nodes->scratch_type == ScratchSSD)
      scratch = -2;
  else if (spec->nodes->scratch_type == ScratchShared) 
      scratch = -2;
